...
const SIZE_LIMITS = {
  product: 5 * 1024 * 1024, // 5MB para productos
  business: 2 * 1024 * 1024, // 2MB para logos de negocios
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verificar el tamaño del archivo según el tipo
    const maxSize = SIZE_LIMITS[type]
    if (file.size > maxSize) {
      const maxSizeMB = maxSize / (1024 * 1024)
      return NextResponse.json(
        {
          error: `El archivo es demasiado grande. El tamaño máximo permitido para ${
            type === "product" ? "productos" : "logos"
          } es de ${maxSizeMB}MB.`,
        },
        { status: 400 },
      )
